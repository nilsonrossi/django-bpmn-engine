...
class DictionarySerializer(Serializer):
    ...
    def serialize_service_task(self, spec):
        s_state = self.serialize_task_spec(spec)
        s_state['topic'] = spec.topic
        s_state['properties'] = spec.properties
        return s_state

    def deserialize_service_task(self, wf_spec, s_state):
        spec = ServiceTask(wf_spec, s_state['name'], s_state['topic'], s_state["properties"])
        self.deserialize_task_spec(wf_spec, s_state, spec=spec)
        return spec
